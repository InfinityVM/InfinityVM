...
cd ..
forge script CoprocessorDeployer.s.sol:CoprocessorDeployer --sig "deployCoprocessorContracts(address relayer, address coprocessorOperator, address offchainSigner, bool writeJson)" $RELAYER $COPROCESSOR_OPERATOR $OFFCHAIN_SIGNER $WRITE_JSON --rpc-url $RPC_URL --private-key $PRIVATE_KEY --chain-id $CHAIN_ID --broadcast -v
