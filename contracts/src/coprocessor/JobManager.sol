    ...
    ReentrancyGuard
{
    ...
    function submitResultForOffchainJob(
        ...
        // Create a job without emitting an event and set onchain input, offchain input hash, and state input hash on consumer
        _createJob(request.nonce, jobID, request.programID, request.maxCycles, request.consumer);
        Consumer(request.consumer).setInputsForJob(jobID, request.onchainInput, request.offchainInputHash, request.stateInputHash);
        ...
        require(request.stateInputHash == result.stateInputHash, "JobManager.submitResultForOffchainJob: state input hash signed by coprocessor doesn't match state input hash of job request");
