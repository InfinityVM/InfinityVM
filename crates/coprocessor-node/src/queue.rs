...
impl Queues {
    ...
    pub fn peek_front(&self, consumer_address: [u8; 20]) -> Option<[u8; 32]> {
...
mod test {
    ...
    fn single_node_queue() {
        ...
        assert!(queue.peek_front(addr0).is_none());
        ...
        assert_eq!(queue.peek_front(addr0), Some(id0));
    ...
    fn push_pop() {
        ...
        assert_eq!(queue.pop_front(addr0), Some(id0));
        assert_eq!(queue.peek_front(addr0), Some(id1));

        assert_eq!(queue.pop_front(addr0), Some(id1));
        assert_eq!(queue.peek_front(addr0), Some(id2));

        assert_eq!(queue.pop_front(addr0), Some(id2));
        assert_eq!(queue.peek_front(addr0), Some(id3));

        assert_eq!(queue.pop_front(addr0), Some(id3));
        assert_eq!(queue.peek_front(addr0), None);
