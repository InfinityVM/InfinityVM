...
use remote_db::remote_db::v1::{StoreElfRequest, GetElfRequest};
use std::fs;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = client
        .get_elf(GetElfRequest {
            program_id: program_id.clone(),
        })
        .await?;
    println!("Retrieved ELF size: {} bytes", response.get_ref().program_elf.len());
    
    assert_eq!(response.get_ref().program_elf, program_elf, "Retrieved ELF doesn't match original!");
