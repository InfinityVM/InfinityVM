...
#[derive(Default, Debug, PartialEq, EnumString)]
enum LogFormat {
    #[default]
    Text,
    Json,
...
pub fn init_logging() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let log_format = LogFormat::from_str(rust_log_format.as_str()).unwrap_or_default();
    // TODO: check for _gaurd
    let (writer, _guard) = if !rust_log_file.is_empty(){
