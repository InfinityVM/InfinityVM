...
pub trait Zkvm {
    ...
    /// the zkvm operator risks slashing because the execution result from the program would deviate
    /// from the zkvm proof result from the elf. Thus, the derivation and persistence of the program
    /// is a highly sensitive process.
    ///
