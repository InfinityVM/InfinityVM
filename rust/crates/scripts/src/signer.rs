...
impl RequestAndResultSigner {
    ...
    pub async fn sign_job_request() {
        ...
        let encoded_job_request = abi_encode_job_request(1, 1_000_000, consumer_addr, b"programID".to_vec(), abi_encode_address(zero_addr));
    ...
    pub async fn sign_offchain_result() {
        ...
        let raw_output = abi_encode_address_with_balance(zero_addr, Uint::from(10));
        let encoded_offchain_result = abi_encode_offchain_result(program_input_hash.into(), 1_000_000, b"programID".to_vec(), raw_output);
...
fn abi_encode_job_request(nonce: u32, max_cycles: u64, consumer: Address, program_id: Vec<u8>, program_input: Vec<u8>) -> Vec<u8> {
...
fn abi_encode_offchain_result(program_input_hash: [u8; 32], max_cycles: u64, program_id: Vec<u8>, result: Vec<u8>) -> Vec<u8> {
